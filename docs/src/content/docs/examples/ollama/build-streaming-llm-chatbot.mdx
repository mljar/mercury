...
Now we have a “container” that we can update as tokens arrive.
...
Without it, the next prompt would not include the assistant’s last reply.
...
* For better answers, keep the `messages` list (conversation history).
  If you remove it, the chatbot becomes “single-turn” (no memory).
