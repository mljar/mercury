...
if input_file.name is None:
    mr.Markdown("# Upload the dataset and check the summary")
else:
    mr.Markdown("# Dataset summary", key="title")
```

Calling `mr.Markdown(...)` directly inside a cell renders it — no `display()` wrapper needed when the Mercury widget is created right there.
...
`clear_output()` hides the cell output entirely when there's nothing to show, so the app doesn't leave empty gaps on first load. `display(ind_basic)` is needed here because `ind_basic` was created earlier in the computation cell — for Mercury widgets created inline (like the markdown headings and the column type table), the bare call is enough.
