...
`mr.Markdown(...)` renders immediately, so no `display()` wrapper is needed.
Assigning its returned widget to `_` prevents IPython from displaying the last
expression a second time. The same pattern appears in every conditional cell
below.
