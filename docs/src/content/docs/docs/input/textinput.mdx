...
The **TextInput** widget displays a single-line or multi-line text input field.
It is useful for entering names, labels, filters, search queries, comments, or any short or multi-line text values in your Mercury App.
...
Set `rows` to an integer greater than `1` to render a multi-line `<textarea>` widget.
...
* Default is `1` (single-line `<input>` element).
* When `rows > 1`, the widget renders as a multi-line `<textarea>` with vertical resizing enabled.
