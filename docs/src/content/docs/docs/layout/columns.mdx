...
Pass a list of positive numbers to create columns with proportional widths.
...
The values are treated as ratios, so `[40, 60]`, `[0.4, 0.6]`, and `[2, 3]`
all work.


### Minimum Width and Responsiveness

No responsive configuration is required. By default, each column targets a minimum
width of 260px. Two columns therefore stay in one row on desktop and tablet layouts,
then stack into two rows on a typical mobile screen. Four columns can progressively
wrap from four to two to one per row as their containing area gets narrower.

Responsiveness is based on the available container width, so it also works for nested
layouts and pages with a sidebar.
...
Each returned column is an output widget and supports `.clear()`.
